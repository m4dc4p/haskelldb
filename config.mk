...
GHCFLAGS += -fglasgow-exts -fallow-overlapping-instances \
            -fwarn-unused-imports $(GHCFLAGS_WARN)
...
HSQL_DIR = $(TOP_DIR)/../HSQL-cvs/HSQL/build
UNIXODBC_PREFIX = /usr

HOST = $(shell hostname)
ifeq ("$(HOST)", "jackass.tekno.chalmers.se")
HSQL_DIR = /usr/local/dp03-7/HSQL-cvs/HSQL/build
UNIXODBC_PREFIX = /usr/local/dp03-7/unixODBC-2.2.6
endif

UNIXODBC_LIBDIR = $(UNIXODBC_PREFIX)/lib
HSQL_LIBDIR = $(HSQL_DIR)

GHCFLAGS += -i$(SRC_DIR) -i$(HSQL_DIR)
LDFLAGS += -L$(SRC_DIR) -L$(HSQL_LIBDIR) -L$(UNIXODBC_LIBDIR)
