...
tInfoToModule dbname tinfo@TInfo{tname=name,cols=col}
    = (modname,
       ...
       $$ ppTable tinfo       
       $$ ppComment ["Fields"]
       $$ if col == [] then empty -- no fields, don't do anything weird
             else vcat (map ppField (columnNamesTypes tinfo)))
    where modname = dbname ++ "." ++ moduleName name

ppTableType :: TInfo -> Doc
ppTableType (TInfo { tname = tiName, cols = tiColumns }) =
    hang decl 4 types
  where
    decl = text "type" <+> text (toType tiName) <+> text "="
    types = ppColumns tiColumns
    
-- | Pretty prints a TableInfo
ppTable :: TInfo -> Doc
ppTable (TInfo tiName tiColumns) =  
    hang (text (identifier tiName) <+> text "::" <+> text "Table") 4 
	 (text (toType tiName) <> newline)
    $$  
