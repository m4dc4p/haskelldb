...
type Calendartime_tbl =
    Record (HCons (LVPair F01 (Expr (Maybe CalendarTime)))
            (HCons (LVPair F02 (Expr CalendarTime))
             (HCons (LVPair F03 (Expr (Maybe CalendarTime)))
              (HCons (LVPair F04 (Expr CalendarTime)) HNil))))
